...
def doctor():
    ...
    result = {"platform": platform.platform(), "machine": platform.machine(), "tools": [probe(t) for t in tools], "manifests": [str(p.relative_to(ROOT)) for p in find_files('Cargo.toml','package.json','pom.xml','build.gradle','build.gradle.kts','Package.swift','pubspec.yaml','*.csproj')]}
...
def project_commands():
    """Return language workspace commands only for manifests actually present."""
    commands = []
    cargo = find_files("Cargo.toml")
    if cargo and which("cargo"):
        root = next((p.parent for p in cargo if (p.parent / "Cargo.lock").exists()), cargo[0].parent)
    ...
    for manifest in find_files("package.json"):
        if not which("npm"): continue
        root = manifest.parent
        commands.append(("node", root, ["npm", "run", "build", "--if-present"]))
        commands.append(("node-test", root, ["npm", "test", "--if-present"]))
    for manifest in find_files("pom.xml"):
        if which("mvn"):
            commands += [("java", manifest.parent, ["mvn", "-B", "package"]), ("java-test", manifest.parent, ["mvn", "-B", "test"])]
    for manifest in find_files("build.gradle", "build.gradle.kts"):
        if which("gradle"):
            commands += [("gradle", manifest.parent, ["gradle", "build"]), ("gradle-test", manifest.parent, ["gradle", "test"])]
    for manifest in find_files("*.csproj"):
        if which("dotnet"):
            commands += [("dotnet", manifest.parent, ["dotnet", "build", manifest.name, "-c", "Release"]), ("dotnet-test", manifest.parent, ["dotnet", "test", manifest.name, "-c", "Release", "--no-build"])]
    for manifest in find_files("Package.swift"):
        if which("swift"):
            commands += [("swift", manifest.parent, ["swift", "build", "-c", "release"]), ("swift-test", manifest.parent, ["swift", "test"])]
    for manifest in find_files("pubspec.yaml"):
        if which("dart"):
            commands += [("dart", manifest.parent, ["dart", "pub", "get"]), ("dart-test", manifest.parent, ["dart", "test"])]
    return commands
...
def install_deps(dry_run=False):
    ...
    if find_files("Cargo.toml") and which("cargo"):
        for p in find_files("Cargo.toml"):
            if p.parent.name == "rust" or (p.parent / "Cargo.lock").exists(): commands.append(["cargo", "fetch", "--locked"] if (p.parent / "Cargo.lock").exists() else ["cargo", "fetch"]); break
    for p in find_files("package-lock.json"):
        commands.append(["npm", "ci"]); break
    for p in find_files("package.json"):
        if not find_files("package-lock.json"): commands.append(["npm", "install"]); break
...
def main(argv=None):
    ...
    if args.command == "build":
        if not args.dry_run and not (build / "CMakeCache.txt").exists():
            ...
            if rc: return rc
        rc = native_build(build, args.jobs, args.dry_run)
        return rc if rc else language_build(args.dry_run)
    if args.command == "test":
        rc = native_test(build, args.dry_run)
        return rc if rc else language_test(args.dry_run)
