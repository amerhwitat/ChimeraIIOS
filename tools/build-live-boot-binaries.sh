...
mkdir -p "$OUT/boot" "$OUT/initramfs/root"/{bin,sbin,dev,proc,sys,run,tmp,mnt/chimera,etc} "$OUT/mobile"/{arm64,armv7,x86_64} "$OUT/manifests"
...
if [[ -n "$KORONOS" && -f "$KORONOS" ]]; then cp -f "$KORONOS" "$OUT/boot/koronos.elf"; sha256sum "$OUT/boot/koronos.elf" > "$OUT/boot/koronos.elf.sha256"; fi
...
if [ -f /mnt/chimera/boot/live-manifest.json ]; then
  ...
  echo "Linux hardware kernel attached."
  echo "Koronos runtime payload: /mnt/chimera/boot/koronos.elf"
else
...
EOF
chmod +x "$INIT/init"
(cd "$INIT" && find . -print0 | cpio --null -o -H newc 2>/dev/null | gzip -9) > "$OUT/boot/chimera-live-initramfs.img"
sha256sum "$OUT/boot/chimera-live-initramfs.img" > "$OUT/boot/chimera-live-initramfs.img.sha256"
...
printf '{"schema":"CHM-LIVE-BOOT-2","firmware":["legacy-bios","uefi"],"bootloader":"GRUB2/El Torito","hardware_kernel":"%s","koronos_payload":"%s","initramfs":"chimera-live-initramfs.img","kernel_attachment":"linux-hardware-kernel + Koronos runtime payload","checksums":true}\n' "$(basename "${KERNEL:-missing}")" "$(basename "${KORONOS:-missing}")" > "$OUT/manifests/live-boot.json"
echo "Live boot artifacts generated in $OUT"
