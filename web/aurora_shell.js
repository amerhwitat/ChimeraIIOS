(()=>{'use strict';
const fallback={categories:[{id:'chimera',title:'Chimera II',apps:[{id:'architecture',title:'Architecture',icon:'A',target:'#architecture'},{id:'isa',title:'ISA Explorer',icon:'◈',target:'#isa-section'},{id:'kernel',title:'Koronos Kernel',icon:'K',target:'#kernel-section'},{id:'terminal',title:'Aurora Terminal',icon:'⌘',target:'#terminal-section'},{id:'desktop-switcher',title:'Desktop Switcher',icon:'▣',url:'desktop_switcher.html'},{id:'retro-emulators',title:'Retro Emulator Center',icon:'🕹',url:'retro_emulators.html'}]}]};
...
async function init(){
 ...
 dock.innerHTML='<b>✦</b><button data-desktop title="Switch desktop">▣</button>'+apps.slice(0,8).map(a=>`<button data-app="${a.id}" title="${a.title}">${a.icon}</button>`).join('')+'<button data-menu="1" title="All applications">⋮⋮</button>';
 ...
 const desktopButton=()=>{window.open('desktop_switcher.html','chimera-desktop-switcher','noopener,noreferrer')};
 dock.addEventListener('click',e=>{const b=e.target.closest('[data-app]');if(b)launch(apps.find(a=>a.id===b.dataset.app));if(e.target.closest('[data-menu]'))menu.hidden=!menu.hidden;if(e.target.closest('[data-desktop]'))desktopButton()});
