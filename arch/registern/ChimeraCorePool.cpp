...
void ChimeraCorePool::submit(std::function<void(std::size_t)> job){
 if(stopping_)throw std::runtime_error("core pool stopped");
 {std::lock_guard lk(state_->m);state_->q.push(std::move(job));} state_->cv.notify_one();
}
void ChimeraCorePool::stop(){if(stopping_.exchange(true))return;state_->cv.notify_all();for(auto&t:workers_)if(t.joinable())t.join();}
}