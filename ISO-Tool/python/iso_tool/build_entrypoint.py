"""Single ISO-Tool build entry point: configure, compile, link and stage artifacts."""
...
def build(source:Path,output:Path,compiler='auto',log=print):
    source=source.resolve(); output=output.resolve(); layout=prepare_output_layout(output); build_root=output/'build'/compiler; build_root.mkdir(parents=True,exist_ok=True)
    ...
    if compiler=='gnu':
        gen='Ninja' if shutil.which('ninja') else 'MinGW Makefiles'; run([cmake,'-S',str(source),'-B',str(build_root),'-G',gen,'-DCMAKE_BUILD_TYPE=Release'],source,log)
    ...
    run([cmake,'--build',str(build_root),'--config','Release','--parallel'],source,log)
    artifacts=[]
    for p in build_root.rglob('*'):
        ...
        if d:
            t=d/p.name; t.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(p,t); artifacts.append(str(t)); log(f'[artifact] {t}')
    m=layout['manifests']/'build-result-'+compiler+'.json'; m.write_text(json.dumps({'source':str(source),'compiler':compiler,'build_tree':str(build_root),'artifacts':artifacts},indent=2),encoding='utf-8'); return artifacts
