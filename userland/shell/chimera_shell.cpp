...
static int cmd_echo(const CommandContext&,int argc,const char* const* argv,char* o,std::size_t n){std::string s;for(int i=1;i<argc;i++){if(i>1)s+=' ';s+=argv[i];}s+='\n';out(o,n,s);return 0;}
...
static int invoke(const char* tool,int argc,const char*const* argv,char*o,std::size_t n){std::string s=tool;for(int i=1;i<argc;i++){s+=" \"";for(char c:std::string(argv[i])){if(c=='\"'||c=='\\')s+='\\';s+=c;}s+="\"";}s+=" 2>&1";FILE*p=std::popen(s.c_str(),"r");if(!p){out(o,n,std::string(tool)+": unavailable\n");return 127;}std::string r;char b[512];while(std::fgets(b,sizeof(b),p))r+=b;int rc=std::pclose(p);out(o,n,r);return rc==0?0:1;}
