...
static int cmd_chmctl(const CommandContext&,int argc,const char*const* argv,char*o,std::size_t n){std::string s="chmctl";for(int i=1;i<argc;i++){s+=" ";s+=argv[i];}s+=" 2>&1";FILE* p=std::popen(s.c_str(),"r");if(!p){out(o,n,"chmctl: unavailable\\n");return 127;}std::string r;char b[512];while(std::fgets(b,sizeof(b),p))r+=b;int rc=std::pclose(p);out(o,n,r);return rc==0?0:1;}
static int cmd_settings(const CommandContext&,int argc,const char*const* argv,char*o,std::size_t n){std::string s="aurora-settings";for(int i=1;i<argc;i++){s+=" ";s+=argv[i];}s+=" 2>&1";FILE* p=std::popen(s.c_str(),"r");if(!p){out(o,n,"aurora-settings: unavailable\\n");return 127;}std::string r;char b[512];while(std::fgets(b,sizeof(b),p))r+=b;std::pclose(p);out(o,n,r);return 0;}
static int cmd_diagnostics(const CommandContext&,int argc,const char*const* argv,char*o,std::size_t n){std::string s="chm-diagnostics";for(int i=1;i<argc;i++){s+=" ";s+=argv[i];}s+=" 2>&1";FILE* p=std::popen(s.c_str(),"r");if(!p){out(o,n,"chm-diagnostics: unavailable\\n");return 127;}std::string r;char b[512];while(std::fgets(b,sizeof(b),p))r+=b;std::pclose(p);out(o,n,r);return 0;}
static int cmd_help(const CommandContext&,int,char*const*,char*o,std::size_t n){out(o,n,"Core: cd pwd echo printf env export set unset alias unalias command type which\\nSystem: chmctl aurora-settings chm-diagnostics chmhelp\\nFilesystem: ls cp mv rm mkdir rmdir touch ln find stat file chmod chown du df\nText: cat head tail less more grep sed awk cut sort uniq tr wc diff tee xargs\nProcess: ps top kill pkill pgrep jobs fg bg wait nice renice\nNetwork: ip ss ping traceroute dig host curl wget ssh scp sftp nc\nSystem: uname hostname date uptime free dmesg mount umount id who whoami\n");return 0;}
