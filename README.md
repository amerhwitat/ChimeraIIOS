...
Chimera II OS is a cross-language research operating-system and application platform centered on the Koronos microkernel, wide-register C8192/R8192 research ISA, multidimensional cognition, portable tooling, trusted peer networking and a separate Mobile Microkernel/application stack.
...
The Kotlin mobile portfolio now includes synchronized text conversations, IRC-style channels, presence/session metadata and a WebRTC-ready voice/video boundary. Android microphone and camera permissions are declared for mobile communication applications and must be requested only when the user starts the corresponding feature. See [`docs/MOBILE_COMMUNICATIONS.md`](docs/MOBILE_COMMUNICATIONS.md).
